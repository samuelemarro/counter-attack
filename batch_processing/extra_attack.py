...
@click.option('--log-dir', type=click.Path(file_okay=False, dir_okay=True), default='extra_attack_logs')
def main(domain, architecture, test_name, attacks, start, log_dir):
    stop = start + 1
    ...
    compare_results_path = f'extra_attack_results/{test_name}/{domain}-{architecture}/{start}-{stop}.zip'
    Path(compare_results_path).parent.mkdir(parents=True, exist_ok=True)
