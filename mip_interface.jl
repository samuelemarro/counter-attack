...
function find_adversarial_example(
    ...
    #=
    starting_output = starting_point |> nn
    starting_predicted_index = starting_output |> MIPVerify.get_max_index
    notice(
            MIPVerify.LOGGER,
            "Julia starting point predicted label is $(starting_predicted_index)",
        )
    starting_linf = maximum(abs.(collect(Iterators.flatten(input - starting_point))))
    notice(
            MIPVerify.LOGGER,
            "Julia starting point linf is $(starting_linf)",
        )
    =#
