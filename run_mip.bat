FOR %%D IN (mnist, cifar10) DO (
    FOR %%A IN (a, b, c) DO (
        python cli.py accuracy %%D %%A std:test --state-dict-path trained-models\best-classifiers\%%D-%%A.pth --stop 10
        ...
        python cli.py attack %%D %%A std:test mip linf --stop 10 --attack-config-file %CONFIG% --state-dict-path trained-models\best-classifiers\%%D-%%A.pth
    )
)